...
/**
 * ExtIO provides a set of static methods for reading and writing text.  By default, it reads
 ...
 * behavior; to handle this case, ExtIO will give up after 10 consecutive illegal inputs and
 ...
 * <p>This class requires Java 5.0 or higher. (A previous version of ExtIO required only Java 1.1;
...
public class ExtIO {
   ...
   /* Modified November 2007 to empty the ExtIO input buffer when switching from one
    ...
    */
    ...
    /**
     * If ExtIO is currently writing to a file, then the return value is the name of the file.
    ...
    public static void putf(String format, Object... items) {
        if (format == null)
            throw new IllegalArgumentException("Null format string in ExtIO.putf() method.");
        try {
            out.printf(format,items);
        }
        catch (IllegalFormatException e) {
            throw new IllegalArgumentException("Illegal format string in ExtIO.putf() method.");
        }
        out.flush();
        if (out.checkError())
            outputError("Error while writing output.");
     ...
     * end-of-file character (specified by the constant ExtIO.EOF).An end-of-line is always returned
    ...
    private static void outputError(String message) {  // Report an error on output.
        if (writingStandardOutput) {
            System.err.println("Error occurred in ExtIO while writing to standard output!!");
            outputErrorCount++;
